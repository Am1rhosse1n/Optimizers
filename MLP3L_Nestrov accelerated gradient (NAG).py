...
for i in range(epochs):
    ...
    for j in range(np.shape(x_train)[0]):
        ...
        #Nestrov accelerate gradient  (NAG)
        w1_temp = w1 - (gama * v1)      # Vt-1
        w2_temp = w2 - (gama * v2)      # Vt-1
        w3_temp = w3 - (gama * v3)      # Vt-1
        ...
        w1 = np.subtract(w1 , (gama * v1) + eta * delta_w1)
        w2 = np.subtract(w2 , (gama * v2) + eta * delta_w2)
        w3 = np.subtract(w3 , (gama * v3) + eta * delta_w3)

        # Update momentum
        v1 = (gama * v1) + (eta * delta_w1)     #Vt
        v2 = (gama * v2) + (eta * delta_w2)     #Vt
        v3 = (gama * v3) + (eta * delta_w3)     #Vt
