...
for i in range(epochs):
    ...
    for j in range(np.shape(x_train)[0]):
        ...
        w1 = np.subtract(w1 , eta/g1 * m1)
        w2 = np.subtract(w2 , eta/g2 * m2)
        w3 = np.subtract(w3 , eta/g3 * m3)

        m1 = (b1 * m1) + ((1-b1) * delta_w1)
        v1 = (b2 * v1) + ((1-b2) * delta_w1 ** 2)
        g1 = math.sqrt(sum(sum(v1)))

        m2 = (b1 * m2) + ((1-b1) * delta_w2)
        v2 = (b2 * v2) + ((1-b2) * delta_w2 ** 2)
        g2 = math.sqrt(sum(sum(v2)))

        m3 = (b1 * m3) + ((1-b1) * delta_w3)
        v3 = (b2 * v3) + ((1-b2) * delta_w3 ** 2)
        g3 = math.sqrt(sum(sum(v3)))

#        rms1 = (gamma * rms1) + ( 1-gamma * (delta_w1 ** 2)) 
#        g1 = math.sqrt(sum(sum(rms1)))
#        rms2 = (gamma * rms2) + ( 1-gamma * (delta_w2 ** 2))
#        g2 = math.sqrt(sum(sum(rms2)))
#        rms3 = (gamma * rms3) + ( 1-gamma * (delta_w3 ** 2))
